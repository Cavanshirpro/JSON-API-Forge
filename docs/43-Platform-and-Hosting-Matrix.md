...
| Windows 10/11 and Windows Server | `windows-*` | x64, ARM64 | `forge-server.exe`, optionally a Windows service | Terminate TLS in IIS, Caddy, nginx or another trusted reverse proxy; configure its exact CIDR. |
...
The GitHub workflow creates checksums beside every archive. `scripts/install-release.sh` and `scripts/install-release.ps1` select only known asset names, download over HTTPS, validate SHA-256, and refuse an existing destination. They never fetch or execute a platform value supplied by a remote response.
