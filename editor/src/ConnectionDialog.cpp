...
void ConnectionDialog::validateAndAccept()
{
    ...
    if (mode == AuthenticationMode::JoinInvitation
        && (invitation().size() < 32 || invitation().size() > 512 || invitation().contains(u'\n')
            || invitation().contains(u'\r'))) {
        m_error->setText(QStringLiteral("Enter the complete single-use invitation token."));
        m_error->show();
        return;
    }
    const auto token = setupToken();
    if (mode == AuthenticationMode::FounderSetup
        && (token.size() < 32 || token.size() > 512 || token.contains('\n') || token.contains('\r'))) {
        m_error->setText(QStringLiteral("Setup token must contain 32–512 characters without line breaks."));
        m_error->show();
        return;
    }
    accept();
