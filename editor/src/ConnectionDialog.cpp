    ...
    , m_description(new QLabel(this))
{
    ...
    m_server->setDuplicatesEnabled(false);
    QStringList serverHistory;
    if (!lastServer.isEmpty()) {
        serverHistory.append(lastServer.toString());
    }
    for (const auto &entry : QSettings().value(QStringLiteral("connection/serverHistory")).toStringList()) {
        if (!serverHistory.contains(entry)) {
            serverHistory.append(entry);
        }
    }
    m_server->addItems(serverHistory);
    m_server->setCurrentText(lastServer.toString());
    m_server->lineEdit()->setPlaceholderText(QStringLiteral("https://forge.example.com"));
    m_server->lineEdit()->setClearButtonEnabled(true);
...
{
    return QUrl::fromUserInput(m_server->currentText().trimmed());
}
...
{
    return m_setupToken->text().trimmed().toUtf8();
}
...
void ConnectionDialog::setInitialMode(AuthenticationMode mode, const QString &username)
{
    const auto index = m_mode->findData(static_cast<int>(mode));
    if (index >= 0) {
        m_mode->setCurrentIndex(index);
    }
    m_username->setText(username.trimmed());
...
void ConnectionDialog::validateAndAccept()
{
    ...
    if (mode == AuthenticationMode::FounderSetup && setupTokenInvalid) {
        m_error->setText(QStringLiteral(
            "Setup token must contain 32–512 printable ASCII characters. Surrounding whitespace or CR/LF is ignored; whitespace inside the token is not allowed."));
        m_error->show();
        return;
    }
    accept();
