...
bool DocumentCodec::isSafeDocumentPath(const QString &path, bool allowHooks)
{
    if (path.isEmpty() || path.startsWith(u'/') || path.contains(u'\\') || path.contains(u'%')
        || path.contains(QChar::Null) || path.contains(u'\r') || path.contains(u'\n')) {
        return false;
    }
    ...
    static const QRegularExpression JsonNamePattern(
        QStringLiteral(R"(^[A-Za-z0-9](?:[A-Za-z0-9._-]{0,126}[A-Za-z0-9])?\.json$)"));
    static const QRegularExpression HookNamePattern(
        QStringLiteral(R"(^[A-Za-z0-9](?:[A-Za-z0-9._-]{0,126}[A-Za-z0-9])?\.py$)"));
    if (parts.at(0) == QStringLiteral("config")) {
        return JsonNamePattern.match(parts.at(1)).hasMatch();
    }
    ...
    return allowHooks && parts.at(0) == QStringLiteral("hooks")
        && HookNamePattern.match(parts.at(1)).hasMatch();
}
