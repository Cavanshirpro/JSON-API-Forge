...
public:
    ...
    void setInteractionSettings(bool rightDragPan, double zoomSensitivity)
    {
        cancelInteractions();
        m_rightDragPan = rightDragPan;
        m_zoomSensitivity = std::isfinite(zoomSensitivity) ? qBound(0.25, zoomSensitivity, 3.0) : 1.0;
    }
...
protected:
    ...
    void wheelEvent(QWheelEvent *event) override
    {
        ...
        const qreal step = 1.0 + (0.12 * m_zoomSensitivity);
        const qreal factor = event->angleDelta().y() > 0 ? step : 1.0 / step;
    ...
    void mousePressEvent(QMouseEvent *event) override
    {
        ...
        if (event->button() == Qt::RightButton && m_rightDragPan) {
            m_rightPressed = true;
            m_rightPress = event->position().toPoint();
            m_panStart = m_rightPress;
            event->accept();
            return;
        }
    ...
    {
        if ((m_panning && (event->buttons() & m_panButton) == 0)
            || (m_wireSource != nullptr && (event->buttons() & Qt::LeftButton) == 0)) {
            cancelInteractions();
        }
        if (m_rightPressed && (event->buttons() & Qt::RightButton) != 0
            && !m_panning
            && (event->position().toPoint() - m_rightPress).manhattanLength()
    ...
    void mouseReleaseEvent(QMouseEvent *event) override
    {
        ...
        if (event->button() == Qt::RightButton && m_rightPressed) {
            ...
            if (contextMenuRequested) {
                contextMenuRequested(viewport()->mapToGlobal(event->position().toPoint()));
            }
            event->accept();
            return;
        }
        if (event->button() == Qt::LeftButton && m_wireSource != nullptr) {
            ...
            const auto sourceId = m_wireSource->nodeId();
            const auto targetId = target != nullptr && target->inputHit(scenePosition) ? target->nodeId() : QString();
            // The callback can rebuild the entire scene. Release all scene pointers first.
            cancelInteractions();
            event->accept();
            if (!targetId.isEmpty() && nodesConnected) {
                nodesConnected(sourceId, targetId);
            }
            return;
        }
...
private:
    ...
    bool m_rightDragPan = true;
    double m_zoomSensitivity = 1.0;
    ...
    , m_apply(new QPushButton(QStringLiteral("Apply node properties"), this))
{
    ...
    auto *hint = new QLabel(QStringLiteral("Drag nodes · pull output → input · wheel zoom · right/middle-drag pan"), toolbar);
    ...
    view->contextMenuRequested = [this](const QPoint &globalPosition) {
        QMenu menu(this);
        auto *fit = menu.addAction(QStringLiteral("Fit graph"));
        auto *layout = menu.addAction(QStringLiteral("Auto layout"));
        auto *remove = menu.addAction(QStringLiteral("Delete selection"));
        remove->setEnabled(!m_scene->selectedItems().isEmpty());
        const auto *selected = menu.exec(globalPosition);
        if (selected == fit) {
            fitGraph();
...
void NodeGraphEditor::setInteractionSettings(bool rightDragPan, double zoomSensitivity)
{
    static_cast<GraphView *>(m_view)->setInteractionSettings(rightDragPan, zoomSensitivity);
}
...
void NodeGraphEditor::showCompiledPreview()
{
    ...
    ForgeEditorUi::resizeToFit(&dialog, QSize(760, 620));
