...
async def balance(interaction: discord.Interaction):
    result = await economy.balance(str(interaction.user.id))
    await interaction.response.send_message(f"Balance: {result}", ephemeral=True)
...
async def pay(interaction: discord.Interaction, member: discord.Member, amount: app_commands.Range[int, 1, 1_000_000]):
    # The client never sends SQL. It sends domain inputs to the predeclared economy.transfer operation.
    result = await economy.transfer(
        str(interaction.user.id), str(member.id), amount,
        interaction_id=str(interaction.id),
    )
    await interaction.response.send_message(f"Transfer complete: {result}", ephemeral=True)
...
try:
    ...
    # For long-lived apps, arrange service shutdown in your application's normal async lifecycle.
    pass
