...
class EditorIdentityStore:
    ...
    async def authenticate(self, token: str, user_agent: str) -> EditorPrincipal:
        if not token.startswith("jfe_session_") or len(token) > 512 or any(character in token for character in "\r\n\0"):
    ...
    def _validated_role_values(self, values: dict[str, Any], caller: EditorAccess) -> dict[str, Any]:
        ...
        if not caller.principal.is_founder and any(not caller.permits(item) for item in permissions):
            raise HTTPException(status_code=403, detail="A role cannot grant permissions the caller does not hold")
        if not caller.principal.is_founder:
            if any(not any(fnmatch.fnmatchcase(pattern, parent) for parent in caller.document_allow) for pattern in allow):
                raise HTTPException(status_code=403, detail="A role cannot widen the caller's document scope")
            if any(not any(fnmatch.fnmatchcase(pattern, parent) for parent in caller.database_allow) for pattern in databases):
                raise HTTPException(status_code=403, detail="A role cannot widen the caller's database scope")
    ...
    async def save_role(self, caller: EditorAccess, values: dict[str, Any], role_id: str | None = None) -> dict[str, Any]:
        ...
        try:
            async with self.engine.begin() as connection:
                if role_id:
                    current = (await connection.execute(select(editor_roles).where(editor_roles.c.id == role_id))).mappings().first()
                    if not current:
                        raise HTTPException(status_code=404, detail="Role not found")
                    if current["system"] or int(current["rank"]) >= caller.rank:
    ...
    async def create_invitation(self, caller: EditorAccess, memberships: list[dict[str, str]], expires_hours: int) -> str:
        ...
        for membership in memberships:
            ...
            if not role or role["rank"] >= caller.rank or _PROJECT_SCOPE.fullmatch(project) is None:
    ...
    async def accept_invitation(
        ...
        try:
            async with self.engine.begin() as connection:
                invitation_row = (
                    (await connection.execute(select(editor_invitations).where(editor_invitations.c.token_hash == digest)))
                    .mappings()
                    .first()
                )
                ...
                for membership in json.loads(invitation_row["memberships"]):
                    await connection.execute(
                        insert(editor_memberships).values(
                            id=str(uuid.uuid4()),
                            user_id=user_id,
    ...
    async def replace_memberships(self, caller: EditorAccess, user_id: str, memberships: list[dict[str, str]], *, active: bool) -> None:
        ...
        for item in memberships:
            ...
            normalized.append({"role_id": role["id"], "project": project})
        from sqlalchemy import func

        async with self.engine.connect() as connection:
            current_rank = await connection.scalar(
                select(func.max(editor_roles.c.rank))
                .join(editor_memberships, editor_memberships.c.role_id == editor_roles.c.id)
                .where(editor_memberships.c.user_id == user_id)
            )
        if int(current_rank or 0) >= caller.rank or target_rank >= caller.rank:
