...
class EventHub:
    ...
    Network I/O is intentionally decoupled from publish. Each WebSocket owns a
    bounded outbound queue and sender task, so one slow client cannot serialize or
    stall every publisher. This is not a durable message broker: overflow is dropped
    and made observable through `dropped_messages` and logs.
    """
    ...
    def _record_drop(self, channel: str) -> None:
        ...
        # Log at powers of two to make sustained overload visible without turning the
        # overload itself into a logging DoS.
    ...
    async def _socket_sender(self, channel: str, client: _SocketClient) -> None:
        try:
            ...
            # Reader/finally path will also disconnect. Removing here prevents future
            # publishes from feeding a dead socket while receive_text is still waking.
            async with self._lock:
...
class RedisEventHub(EventHub):
    ...
    async def publish(self, channel: str, event: Any) -> int:
        ...
        # Redis returns subscriber count, not delivery acknowledgement. The API calls
        # this a fanout hint, not a durable-delivery count.
