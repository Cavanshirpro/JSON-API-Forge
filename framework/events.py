...
class EventHub:
    ...
    def __init__(self):
        ...
        self._lock=asyncio.Lock(); self.dropped_messages=0
    ...
    async def publish(self,channel:str,event:Any)->int:
        ...
        fanout=0
        for queue in queues:
            try: queue.put_nowait(event); fanout+=1
            except asyncio.QueueFull: self._record_drop(channel)
        for client in sockets:
            try: client.queue.put_nowait(event); fanout+=1
            except asyncio.QueueFull: self._record_drop(channel)
        return fanout
    ...
    async def _socket_sender(self,channel:str,client:_SocketClient)->None:
        try:
            while True:
                event=await client.queue.get(); await client.websocket.send_json(event)
        except asyncio.CancelledError: raise
        except Exception:
            async with self._lock: self._websockets[channel].discard(client)
    ...
    async def connect_ws(self,channel:str,websocket:WebSocket,queue_size:int=256,max_connections:int|None=None)->None:
        ...
        async with self._lock: self._websockets[channel].add(client)
        ...
        async with self._lock:
            for candidate in list(self._websockets[channel]):
                if candidate.websocket is websocket:
                    client=candidate; self._websockets[channel].discard(candidate); break
        if client and client.sender:
            client.sender.cancel(); await asyncio.gather(client.sender,return_exceptions=True)

    async def ping(self)->bool: return True
    async def close(self)->None:
        clients=[client for values in self._websockets.values() for client in values]
        for client in clients:
            if client.sender: client.sender.cancel()
        if clients: await asyncio.gather(*(client.sender for client in clients if client.sender),return_exceptions=True)
        self._websockets.clear(); self._subscribers.clear()
...
class RedisEventHub(EventHub):
    def __init__(self,url:str,prefix:str,project_slug:str):
        ...
        self.prefix=f"{prefix}:{project_slug}"; self._listeners={}; self._listener_ready={}; self._listener_lock=asyncio.Lock()

    def _redis_channel(self,channel:str)->str: return f"{self.prefix}:{channel}"

    async def _ensure_listener(self,channel:str)->None:
        async with self._listener_lock:
            task=self._listeners.get(channel); ready=self._listener_ready.get(channel)
            if not task or task.done():
                ready=asyncio.Event(); self._listener_ready[channel]=ready
                task=asyncio.create_task(self._listen(channel,ready),name=f"forge-redis-event-{channel}"); self._listeners[channel]=task
        ...
        try:
            ...
            if task in done and not ready.is_set(): task.result()
            if ready_waiter in done and ready_waiter.result(): return
            if ready.is_set(): return
            raise RuntimeError(f"Redis event listener did not become ready for channel {channel!r}")
        finally:
            if not ready_waiter.done():
                ready_waiter.cancel(); await asyncio.gather(ready_waiter,return_exceptions=True)

    async def _listen(self,channel:str,ready:asyncio.Event)->None:
        pubsub=self.redis.pubsub(); await pubsub.subscribe(self._redis_channel(channel)); ready.set()
        try:
            ...
            ready.set()
            try: await pubsub.unsubscribe(self._redis_channel(channel))
    ...
    async def subscribe(self,channel:str,queue_size:int,max_subscribers:int|None=None)->AsyncIterator[Any]:
        await self._ensure_listener(channel)
        async for event in super().subscribe(channel,queue_size,max_subscribers=max_subscribers): yield event

    async def connect_ws(self,channel:str,websocket:WebSocket,queue_size:int=256,max_connections:int|None=None)->None:
        await self._ensure_listener(channel); await super().connect_ws(channel,websocket,queue_size=queue_size,max_connections=max_connections)

    async def ping(self)->bool: return bool(await self.redis.ping())
    async def close(self)->None:
        for task in self._listeners.values(): task.cancel()
        if self._listeners: await asyncio.gather(*self._listeners.values(),return_exceptions=True)
        self._listeners.clear(); self._listener_ready.clear(); await super().close(); await self.redis.aclose()
