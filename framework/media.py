...
async def _ensure_usage_row(engine, project_slug: str, owner_subject: str) -> None:
    ...
    async with engine.connect() as conn:
        exists = (await conn.execute(select(media_usage_table.c.used_bytes).where(
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(insert(media_usage_table).values(
        ...
        # Another worker won the first-use race.
        return
...
async def save_media(*, engine, store: LocalMediaStore, project_slug: str, config: MediaConfig, upload: UploadFile, owner_subject: str):
    ...
    if config.deduplicate:
        dedup_filter = (media_table.c.project_slug == project_slug) & (media_table.c.sha256 == digest)
        if config.deduplicate_scope == "owner":
            dedup_filter = dedup_filter & (media_table.c.owner_subject == owner_subject[:160])
        async with engine.connect() as conn:
            existing = (await conn.execute(select(media_table).where(dedup_filter))).mappings().first()
        if existing:
...
def media_api_meta(meta: dict, *, include_owner: bool = False) -> dict:
    ...
    result = {field: meta.get(field) for field in fields}
    if include_owner:
...
async def delete_media(engine, store: LocalMediaStore, project_slug: str, media_id: str):
    ...
    async with engine.begin() as conn:
        result = await conn.execute(delete(media_table).where(
            (media_table.c.id == media_id) & (media_table.c.project_slug == project_slug)
        ))
        if not result.rowcount:
            raise HTTPException(status_code=404, detail="Media not found")
        await conn.execute(update(media_usage_table).where(
            (media_usage_table.c.project_slug == project_slug)
            & (media_usage_table.c.owner_subject == row["owner_subject"])
        ).values(
            used_bytes=case(
                (media_usage_table.c.used_bytes >= row["size"], media_usage_table.c.used_bytes - row["size"]),
                else_=0,
            ),
            updated_at=datetime.now(timezone.utc),
        ))
    try:
        ...
        # Metadata is already gone, so the object is no longer API-addressable. Keep
        # the request successful but surface the orphan for operator cleanup.
