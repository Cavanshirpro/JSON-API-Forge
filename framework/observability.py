...
def observe_audit_drop(count: int = 1) -> None:
    if _AUDIT_DROPPED is not None: _AUDIT_DROPPED.inc(count)


def observe_audit_write_failure(count: int = 1) -> None:
    if _AUDIT_WRITE_FAILURES is not None: _AUDIT_WRITE_FAILURES.inc(count)


def observe_audit_queue(size: int) -> None:
    if _AUDIT_QUEUE is not None: _AUDIT_QUEUE.set(size)


def metrics_payload() -> tuple[bytes, str]:
    if generate_latest is None: return b"prometheus-client is not installed\n", "text/plain"
