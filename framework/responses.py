...
from fastapi.responses import FileResponse, HTMLResponse, ORJSONResponse, PlainTextResponse, RedirectResponse, Response, StreamingResponse
...
def render_response(spec: ResponseSpec, result: Any):
    ...
    if spec.kind == "json":
        return ORJSONResponse(result, status_code=status, headers=headers)
    if spec.kind == "text":
