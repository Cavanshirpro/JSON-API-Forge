...
from ..security import (
    authenticate_request, create_api_key, ensure_credential_delegation, get_api_key_metadata, has_permission, issue_jwt,
...
def register_project_routes(*, app, runtime, principal_for, require, hide_internal_signature, hidden_route, invoke_hook) -> None:
    ...
    if project.docs_enabled:
        ...
        async def project_openapi(_prefix=prefix, _project=project):
            ...
            # Alternatives: API key OR bearer token. Endpoint-level OpenAPI overrides use
            # security=[] only when the declarative config explicitly opts into public access.
    ...
    async def admin_create_key(request: Request, body: APIKeyCreate, _rt=runtime, _project=project):
        ...
        ensure_credential_delegation(
            _project, principal, roles=body.roles, permissions=body.permissions, tenant_id=body.tenant_id,
            expires_at=body.expires_at, rate_requests=body.rate_requests,
            rate_window_seconds=body.rate_window_seconds, rate_burst=body.rate_burst,
        )
        one_time_bootstrap = principal.kind == "bootstrap" and _project.security.bootstrap_one_time
        created = await create_api_key(
            app.state.internal_engine, project_slug=_project.slug, name=body.name, roles=body.roles, permissions=body.permissions,
            expires_at=body.expires_at, tenant_id=body.tenant_id, rate_requests=body.rate_requests,
            rate_window_seconds=body.rate_window_seconds, rate_burst=body.rate_burst, consume_bootstrap_once=one_time_bootstrap,
        )
        if one_time_bootstrap:
    ...
    async def admin_revoke_key(key_id: int, request: Request, _rt=runtime, _project=project):
        ...
        ensure_credential_delegation(
            _project, principal, roles=target["roles"], permissions=target["permissions"],
            tenant_id=target["tenant_id"], expires_at=target["expires_at"],
            rate_requests=target["rate_requests"], rate_window_seconds=target["rate_window_seconds"],
    ...
    if project.security.jwt_enabled and project.security.jwt_provider == "local_hs256":
        ...
        async def admin_issue_jwt(request: Request, body: JWTCreate, _rt=runtime, _project=project):
            ...
            ensure_credential_delegation(
                _project, principal, roles=body.roles, permissions=body.permissions, tenant_id=body.tenant_id,
                expires_at=expires_at, subject=body.subject,
            )
            return {
                "token": issue_jwt(
                    body.subject, _project.slug, body.roles, body.permissions, exp_minutes, body.tenant_id,
    ...
    def register_resource(resource: ResourceConfig):
        ...
        if "list" in resource.allowed_actions:
            async def list_handler(request: Request, _r=resource, _tk=table_key, _rt=runtime, _ns=namespace):
                ...
                if cache_enabled("list"):
                    key = await _rt.cache.make_key(_ns, {
                        "kind": "list", "query": sorted(request.query_params.multi_items()), "tenant": p.tenant_id,
        ...
        if "read" in resource.allowed_actions:
            async def read_handler(item_id: str, request: Request, _r=resource, _tk=table_key, _rt=runtime, _ns=namespace):
                ...
                if cache_enabled("read"):
                    key = await _rt.cache.make_key(_ns, {
                        "kind": "read", "id": item_id, "tenant": p.tenant_id,
                    ...
                    value, hit = await _rt.cache.get_or_set_json(key, ttl("read"), loader, _r.cache.stale_ttl_seconds if _r.cache.stale_ttl_seconds is not None else project.cache.stale_ttl_seconds)
                    request.state.forge_cache = "hit" if hit else "miss"
                    return value
        ...
        if "update" in resource.allowed_actions:
            async def patch_handler(item_id: str, request: Request, payload: dict[str, Any] = Body(...), _r=resource, _tk=table_key, _rt=runtime, _ns=namespace):
                p = await principal_for(request, _rt); require(p, _r.permissions.get("update", f"{_r.path}.update"))
                value = await update_row(_rt.registry.engines[_r.database], _rt.registry.tables[_tk], _r, p, item_id, payload, replace=False)
                if _rt.cache: await _rt.cache.invalidate_namespace(_ns)
                return value
            async def put_handler(item_id: str, request: Request, payload: dict[str, Any] = Body(...), _r=resource, _tk=table_key, _rt=runtime, _ns=namespace):
                p = await principal_for(request, _rt); require(p, _r.permissions.get("update", f"{_r.path}.update"))
                value = await update_row(_rt.registry.engines[_r.database], _rt.registry.tables[_tk], _r, p, item_id, payload, replace=True)
                if _rt.cache: await _rt.cache.invalidate_namespace(_ns)
                return value
            add_route(base + "/{item_id}", patch_handler, methods=["PATCH"], tags=[tag], name=f"{project.slug}_patch_{resource.path}", dependencies=fastapi_dependencies(resource.dependencies))
            add_route(base + "/{item_id}", put_handler, methods=["PUT"], tags=[tag], name=f"{project.slug}_put_{resource.path}", dependencies=fastapi_dependencies(resource.dependencies))
    ...
    def register_mongo_resource(resource):
        ...
        if "read" in resource.allowed_actions:
            async def mongo_read(item_id: str, request: Request, _r=resource, _rt=runtime, _ns=namespace):
                ...
                if mongo_cache_enabled("read"):
                    ...
                    value, hit = await _rt.cache.get_or_set_json(key, mongo_ttl("read"), loader, _r.cache.stale_ttl_seconds if _r.cache.stale_ttl_seconds is not None else project.cache.stale_ttl_seconds)
                    request.state.forge_cache = "hit" if hit else "miss"
                    return value
        ...
        if "update" in resource.allowed_actions:
            ...
            async def mongo_put(item_id: str, request: Request, payload: dict[str, Any] = Body(...), _r=resource, _rt=runtime, _ns=namespace):
                ...
                if _rt.cache: await _rt.cache.invalidate_namespace(_ns)
                return value
            add_route(base + "/{item_id}", mongo_patch, methods=["PATCH"], tags=[tag], name=f"{project.slug}_mongo_patch_{resource.path}", dependencies=fastapi_dependencies(resource.dependencies))
            add_route(base + "/{item_id}", mongo_put, methods=["PUT"], tags=[tag], name=f"{project.slug}_mongo_put_{resource.path}", dependencies=fastapi_dependencies(resource.dependencies))
    ...
    for index, operation in enumerate(project.operations):
        ...
        async def operation_handler(request: Request, background_tasks: BackgroundTasks, payload: Any = Body(default=None), _op=operation, _rt=runtime, _ns=namespace):
            ...
            for hook_path in _op.background_hooks:
                background_tasks.add_task(invoke_hook, import_callable(hook_path), request=request, payload=payload, principal=p, result=result, app=app, project=project)
            return result
    ...
    for index, source in enumerate(project.data_sources):
        ...
        if source.writable:
            async def data_create(request: Request, payload: dict[str, Any] = Body(...), _s=source, _rt=runtime, _ns=namespace):
                ...
                if not _s.public_write: require(p, _s.write_permission or _s.permission)
                value = await _rt.data_sources.create(_s, payload)
                if _rt.cache: await _rt.cache.invalidate_namespace(_ns)
                return value
            async def data_patch(item_id: str, request: Request, payload: dict[str, Any] = Body(...), _s=source, _rt=runtime, _ns=namespace):
                p = await principal_for(request, _rt)
                if not _s.public_write: require(p, _s.write_permission or _s.permission)
            ...
            add_route(base, data_create, methods=["POST"], tags=[f"{project.slug}:data"], name=f"{project.slug}_data_create_{index}", dependencies=fastapi_dependencies(source.dependencies), openapi_extra=write_openapi)
            add_route(base + "/{item_id}", data_patch, methods=["PATCH"], tags=[f"{project.slug}:data"], name=f"{project.slug}_data_patch_{index}", dependencies=fastapi_dependencies(source.dependencies), openapi_extra=write_openapi)
            add_route(base + "/{item_id}", data_put, methods=["PUT"], tags=[f"{project.slug}:data"], name=f"{project.slug}_data_put_{index}", dependencies=fastapi_dependencies(source.dependencies), openapi_extra=write_openapi)
            add_route(base + "/{item_id}", data_delete, methods=["DELETE"], tags=[f"{project.slug}:data"], name=f"{project.slug}_data_delete_{index}", dependencies=fastapi_dependencies(source.dependencies), openapi_extra=write_openapi)
    ...
    for index, channel in enumerate(project.event_channels):
        ...
        async def publish_event(request: Request, payload: Any = Body(...), _c=channel, _rt=runtime):
            ...
            fanout_hint = await _rt.event_hub.publish(_c.name, {"channel": _c.name, "event": payload, "publisher": p.subject, "request_id": request.state.request_id})
            return {"published": True, "fanout_hint": fanout_hint, "delivery": "best-effort"}
        publish_openapi = {"security": []} if channel.public_publish else None
        add_route(base, publish_event, methods=["POST"], tags=[f"{project.slug}:events"], name=f"{project.slug}_event_publish_{index}", openapi_extra=publish_openapi)
        ...
        if channel.sse_enabled:
            ...
            subscribe_openapi = {"security": []} if channel.public_subscribe else None
            add_route(base + "/stream", event_stream, methods=["GET"], tags=[f"{project.slug}:events"], name=f"{project.slug}_event_sse_{index}", openapi_extra=subscribe_openapi)

        if channel.websocket_enabled:
            async def websocket_endpoint(websocket: WebSocket, _c=channel, _rt=runtime, _project=project):
                try:
                    trusted_proxies = _project.protection.trusted_proxy_cidrs
                    ...
                    if _project.security.require_https and not request_is_https(websocket, trusted_proxies):
                        raise HTTPException(status_code=400, detail="HTTPS is required")
                    if not ip_allowed(websocket, _project.security.allowed_ips, _project.security.denied_ips, trusted_proxies):
                    ...
                    effective_ip = client_ip(websocket, trusted_proxies)
    ...
    if project.media.enabled:
        ...
        async def media_batch_upload(request: Request, background_tasks: BackgroundTasks, files: list[UploadFile] = File(...), _rt=runtime, _project=project):
            ...
            for file in files:
                try:
                    ...
                    safe = media_api_meta(item)
                    items.append({"ok": True, "media": safe})
                    succeeded += 1
                    for hook_path in _project.media.post_upload_hooks:
                        background_tasks.add_task(invoke_hook, import_callable(hook_path), media=item, principal=p, app=app, project=_project)
            ...
            return ORJSONResponse(payload, status_code=200 if succeeded == len(items) else 207)
        ...
        async def media_meta(media_id: str, request: Request, _rt=runtime, _project=project):
            ...
            include_owner = p.subject == meta["owner_subject"] or has_permission(p, _project.media.admin_permission)
            return media_api_meta(meta, include_owner=include_owner)
        ...
        async def media_signed_url(media_id: str, request: Request, _rt=runtime, _project=project):
            ...
            token = make_signed_media_token(_project.slug, media_id, _project.media.signed_url_ttl_seconds, secret=_project.media.signing_secret or "")
        ...
        async def media_get(media_id: str, request: Request, token: str | None = None, _rt=runtime, _project=project):
            p = await principal_for(request, _rt)
            signed = _project.media.signed_urls_enabled and verify_signed_media_token(_project.slug, media_id, token, secret=_project.media.signing_secret or "")
