    ...
    if raw_key:
        ...
        if row is None:
            ...
            if row and row["enabled"]:
                expires_at = row.get("expires_at")
                normalized = expires_at if not expires_at or expires_at.tzinfo else expires_at.replace(tzinfo=UTC)
                if not normalized or normalized > datetime.now(UTC):
...
def ensure_credential_delegation(
    ...
    *,
    ...
    if parent.expires_at:
        ...
        pexp = parent.expires_at if parent.expires_at.tzinfo else parent.expires_at.replace(tzinfo=UTC)
        cexp = expires_at if expires_at.tzinfo else expires_at.replace(tzinfo=UTC)
        if cexp > pexp:
