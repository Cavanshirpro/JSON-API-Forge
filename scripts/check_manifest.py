    ...
    try:
        ...
        for p in ROOT.rglob("*"):
            ...
            if rel in ignored or any(
                part in {"__pycache__", ".pytest_cache", ".ruff_cache", ".mypy_cache", "dist", "build", ".git"}
                for part in p.relative_to(ROOT).parts
            ):
                continue
            if rel.endswith((".pyc", ".pyo")) or rel.startswith("data/") or rel.startswith("media/") or rel.startswith("logs/"):
                continue
            out.add(rel)
        return out
