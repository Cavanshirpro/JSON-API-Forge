...
EXAMPLES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "domain": "logistics",
        "entity": "shipments",
        "statuses": ["created", "in_transit", "at_hub", "out_for_delivery", "delivered", "exception"],
...
def _permissions(prefix: str, suffix: str, actions: tuple[str, ...] = ("list", "read", "create", "update", "delete")) -> dict[str, str]:
...
def _resource(
    ...
    *,
    ...
    resource: dict[str, Any] = {
        ...
        "allowed_sort": sort,
        "permissions": _permissions(domain, suffix.replace("/", "."), tuple(actions or ["list", "read", "create", "update", "delete"])),
    }
...
def _primary_resource(definition: dict[str, Any]) -> dict[str, Any]:
    ...
    columns = {
        ...
        "external_id": _column("string", nullable=False, unique=True, index=True, max_length=96),
        "name": _column("string", nullable=False, index=True, max_length=160),
        "status": _column("string", nullable=False, default=statuses[0], index=True, max_length=32),
    ...
    if definition.get("plugin_catalog"):
        columns.update(
            {
                "plugin_id": _column("string", nullable=False, index=True, max_length=128),
                "version": _column("string", nullable=False, index=True, max_length=64),
                "publisher": _column("string", nullable=False, index=True, max_length=128),
                "platform": _column("string", nullable=False, default="any", index=True, max_length=64),
        ...
        properties.update(
            {
                "plugin_id": {"type": "string", "pattern": "^[a-z0-9]+(?:[.-][a-z0-9]+)*$", "maxLength": 128},
                "version": {"type": "string", "minLength": 1, "maxLength": 64},
                "publisher": {"type": "string", "minLength": 1, "maxLength": 128},
                "platform": {"type": "string", "minLength": 1, "maxLength": 64},
                "download_url": {"type": "string", "pattern": "^https://", "maxLength": 1024},
                "sha256": {"type": "string", "pattern": "^[a-fA-F0-9]{64}$"},
                "permissions": {"type": "array", "maxItems": 32, "items": {"type": "string", "maxLength": 96}},
    ...
    resource.update(
        {
            ...
            "create_schema": {"type": "object", "required": required, "additionalProperties": False, "properties": properties},
            "update_schema": {"type": "object", "minProperties": 1, "additionalProperties": False, "properties": properties},
...
def _resources(definition: dict[str, Any]) -> dict[str, Any]:
    workflows = _resource(
        ...
        "workflows",
        {
            ...
            "state": _column("string", nullable=False, default="open", index=True, max_length=32),
            "assigned_to": _column("string", nullable=False, index=True, max_length=96),
            "due_at": _column("datetime", index=True),
            "details": _column("json"),
            "created_at": _column("datetime", index=True),
        },
        writable=["record_id", "step", "state", "assigned_to", "due_at", "details", "created_at"],
    ...
    policies = _resource(
        ...
        {
            "id": _column("integer", primary_key=True, nullable=False),
            "name": _column("string", nullable=False, unique=True, index=True, max_length=128),
    ...
    return {"$schema": SCHEMA, "resources": [_primary_resource(definition), workflows, audits, policies]}
...
def _operations(definition: dict[str, Any]) -> dict[str, Any]:
    ...
    dashboard = {
        ...
        "statements": [
            {
                "sql": f"SELECT status, COUNT(*) AS records, COALESCE(SUM(amount), 0) AS total_amount, COALESCE(AVG(priority), 0) AS average_priority FROM {table} WHERE deleted_at IS NULL GROUP BY status ORDER BY status",
                "mode": "fetch_all",
                "params": {},
    ...
    transition = {
        ...
        "statements": [
            {
                "sql": f"UPDATE {table} SET status = :next_status, updated_at = CURRENT_TIMESTAMP WHERE id = :record_id AND deleted_at IS NULL",
                "mode": "execute",
                "params": {"next_status": "$body.next_status", "record_id": "$body.record_id"},
                "require_rowcount_min": 1,
                "require_rowcount_max": 1,
                "result_name": "transition",
            },
            {
                "sql": f"INSERT INTO {audit_table} (record_id, event_type, actor_id, payload, idempotency_key, created_at) VALUES (:record_id, :event_type, :actor_id, :payload, :idempotency_key, CURRENT_TIMESTAMP)",
                "mode": "execute",
                "params": {
    ...
    assign = {
        ...
        "statements": [
            {
                "sql": f"INSERT INTO {workflow_table} (record_id, step, state, assigned_to, due_at, details, created_at) VALUES (:record_id, :step, 'open', :assigned_to, :due_at, :details, CURRENT_TIMESTAMP)",
                "mode": "execute",
                "params": {
                    "record_id": "$body.record_id",
                    "step": "$body.step",
            ...
            {
                "sql": f"UPDATE {table} SET owner_id = :assigned_to, updated_at = CURRENT_TIMESTAMP WHERE id = :record_id AND deleted_at IS NULL",
                "mode": "execute",
                "params": {"assigned_to": "$body.assigned_to", "record_id": "$body.record_id"},
                "require_rowcount_min": 1,
                "require_rowcount_max": 1,
...
def _graph(definition: dict[str, Any]) -> dict[str, Any]:
    domain = definition["domain"]
    nodes = [
        {"id": "request", "type": "request.input", "title": "Transition request", "x": 0, "y": 0, "properties": {"method": "POST"}},
        {
            "id": "policy",
            "type": "auth.policy",
            ...
            "x": 300,
            "y": 0,
            "properties": {"permission": f"{domain}.{definition['entity']}.transition", "public": False},
        },
        {
            "id": "update",
            "type": "data.mutate",
            ...
            "y": 0,
            "properties": {
                "sql": f"UPDATE {domain}_{definition['entity']} SET status = :next_status WHERE id = :record_id",
                "mode": "execute",
                "params": {"next_status": "$body.next_status", "record_id": "$body.record_id"},
                "result_name": "transition",
            },
        ...
        {"id": "response", "type": "response.output", "title": "Operation result", "x": 1200, "y": 0, "properties": {"status_code": 200}},
    ]
    edges = [
        {"id": "edge-1", "from_node": "request", "from_port": "exec", "to_node": "policy", "to_port": "exec"},
        {"id": "edge-2", "from_node": "policy", "from_port": "exec", "to_node": "update", "to_port": "exec"},
        {"id": "edge-3", "from_node": "update", "from_port": "exec", "to_node": "operation", "to_port": "exec"},
        {"id": "edge-4", "from_node": "operation", "from_port": "exec", "to_node": "response", "to_port": "exec"},
    ]
    return {
        "$schema": "../../../schemas/editor-graph.schema.json",
        "schema_version": 1,
        "target_document": "config/50-operations.json",
        "metadata": {"name": f"{definition['name']} transition", "compiler": "json-api-forge-editor"},
        "nodes": nodes,
        "edges": edges,
    }
...
def _documents(definition: dict[str, Any]) -> dict[str, str]:
    ...
    for suffix in (entity, "workflows", "audit-events", "policies"):
        role_permissions.extend(f"{domain}.{suffix}.{action}" for action in ("list", "read", "create", "update", "delete"))
    ...
    databases = {
        "$schema": SCHEMA,
        "databases": {"primary": {"url": f"$env:{env}_DATABASE_URL:-sqlite+aiosqlite:///./data/{slug}.db", "pool_pre_ping": True}},
    }
    security = {
        "$schema": SCHEMA,
        "security": {
    ...
    runtime = {
        "$schema": SCHEMA,
        "cache": {"enabled": True, "backend": "memory", "default_ttl_seconds": 20, "max_entries": 10000},
        "rate_limit": {
            "enabled": True,
            "backend": "memory",
            "requests": 300,
...
def main() -> None:
    ...
    parser.add_argument("--check", action="store_true", help="Fail if generated examples are missing or stale")
    args = parser.parse_args()
    errors = generate(check=args.check)
    if errors:
        raise SystemExit("Generated example catalog is stale:\n" + "\n".join(errors))
    print(f"OK: {len(EXAMPLES)} large example applications {'verified' if args.check else 'generated'}")
