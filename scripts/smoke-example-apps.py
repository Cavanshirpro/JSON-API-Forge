...
def _validate_editor_schemas() -> None:
    ...
    graph = Draft202012Validator(json.loads(Path("schemas/editor-graph.schema.json").read_text(encoding="utf-8")))
...
def _exercise_generated_app(client: TestClient, values: Mapping[str, str | None], definition: Mapping[str, object]) -> None:
    ...
    assert created.status_code == 200, f"{slug} create: {created.status_code} {created.text}"
    item_id = created.json()["id"]
    listed = client.get(f"{route}?status={statuses[0]}&sort=-priority", headers=auth)
    assert listed.status_code == 200 and any(item["id"] == item_id for item in listed.json()["items"]), listed.text

    dashboard = client.get(f"{prefix}/rpc/{domain}.dashboard", headers=auth)
    assert dashboard.status_code == 200 and "status_summary" in dashboard.json(), dashboard.text
    ...
    transitioned = client.post(f"{prefix}/rpc/{domain}.transition", headers=transition_headers, json=transition_body)
    assert transitioned.status_code == 200, f"{slug} transition: {transitioned.status_code} {transitioned.text}"
    if slug == "commerce-core":
        replay = client.post(f"{prefix}/rpc/{domain}.transition", headers=transition_headers, json=transition_body)
...
def main() -> None:
    ...
    print("OK: 25 example applications passed schema, CRUD, RPC, idempotency, realtime, media and public-data smoke tests")
