...
def test_parameter_sources():
    scope = {
        "type": "http", "method": "POST", "path": "/x/7", "headers": [(b"x-test", b"hello")],
        "query_string": b"page=3", "path_params": {"user_id": "7"}, "server": ("test", 80), "client": ("127.0.0.1", 1234), "scheme": "http"
    }
...
def test_dynamic_internal_bindings_never_become_public_query_parameters():
    ...
    for path, method in [
        ...
        assert not any(p["name"].startswith("_") for p in params)


def test_patch_and_put_have_distinct_operation_ids():
    schema = create_app().openapi()
    route = schema["paths"]["/api/app1/v1/notes/{item_id}"]
